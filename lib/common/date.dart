...
DateTime getOldWeek(){
  ...
  String startTime = formatDate(DateTime.now(),[yyyy, "-", mm, "-", dd]);
  DateTime startData = DateTime.parse(startTime);
  var mothFormatFlag = new DateFormat("yyyy-MM-dd");
  DateTime dateTime = new DateTime.fromMillisecondsSinceEpoch(
      startData.millisecondsSinceEpoch - 7 * 24 * 60 * 60 * 1000);
  //String nowMoth = mothFormatFlag.format(dateTime);
  return dateTime;
}
String  getOldWeekDate(){
  //记录往后每一天的时间搓，用来和最后一天到做对比。这样就能知道什么时候停止了。
  String startTime = formatDate(DateTime.now(),[yyyy, "-", mm, "-", dd]);
  DateTime startData = DateTime.parse(startTime);
  var mothFormatFlag = new DateFormat("yyyy-MM-dd");
